...
type AppSettings struct {
	HubBaseURL string `mapstructure:"hub_base_url"`
}
...
func NewSettingsService() (*SettingsService, error) {
	...
	vp.SetDefault("hub_base_url", "http://localhost:8080")
...
func (s *SettingsService) Save(in *AppSettings) error {
	...
	if err := validateURL(in.HubBaseURL); err != nil {
		return err
	}
	s.vp.Set("hub_base_url", in.HubBaseURL)
...
func (s *SettingsService) Update(partial map[string]any) error {
	...
	// Validate known keys
	if v, ok := partial["hub_base_url"]; ok {
		if str, ok2 := v.(string); ok2 {
			if err := validateURL(str); err != nil {
				return err
			}
			s.vp.Set("hub_base_url", str)
		}
	}
	return s.vp.WriteConfigAs(s.configFile)
}
